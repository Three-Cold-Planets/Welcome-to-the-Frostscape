...
public class SelectOverlay {
    ...
    public void drawSelect(){
        if(selecting) {
            Draw.color(ModPal.select);
            Draw.alpha(0.45f);
            intersectRect();
            Fill.rect(select.x + select.width/2, select.y + select.height/2, select.width, select.height);
            Vars.indexer.eachBlock(Vars.player.team(), select, b -> true, b -> {
                if(!buildings.contains(b)) Drawf.square(b.x, b.y, b.block.size * tilesize / 2f, ModPal.select);
            });
        }
        buildings.each(b -> Drawf.select(b.x, b.y, b.block.size * 4, ModPal.select));
    }
}
